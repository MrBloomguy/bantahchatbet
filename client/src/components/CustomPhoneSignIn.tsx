...
const CustomPhoneSignIn: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const handlePhoneSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // In a real implementation, we would send an SMS with a verification code here
      // For development purposes, we'll simulate it

      // Generate a random OTP
      const generatedOTP = generateOTP();

      // In production, you would store this OTP securely and send it via SMS
      // For development, we'll just log it to the console
      console.log(`Development mode: OTP for ${formattedPhoneNumber} is ${generatedOTP}`);

      // Store the OTP (in production, this would be done securely on the server)
      ...
      // In development mode, show the OTP in a toast for testing
      toast.showInfo(`DEV MODE: Your OTP is ${generatedOTP}`);
  ...
  const handleOtpSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // In a real implementation, we would verify the OTP with a server
      // For development, we'll just compare it with the stored OTP
      if (enteredOTP !== verificationId) {
        throw new Error('Invalid verification code');
      }

      // For development mode, we'll simulate a successful authentication
      // In production, you would implement proper authentication with SMS verification

      try {
        // Create a unique email based on the phone number for Supabase auth
        const phoneEmail = `${phoneNumber.replace(/[^0-9]/g, '')}@phone.user`;
        const phonePassword = `phone-auth-${phoneNumber.replace(/[^0-9]/g, '')}`;
        ...
        if (error) {
          ...
          if (signUpError) {
            // If we get a timeout or network error, we'll use a fallback
            if (signUpError.message?.includes('timeout') ||
                signUpError.message === '{}' ||
                signUpError.status === 504) {
              console.warn('Supabase auth timeout, using fallback authentication');
        ...
        // If we have a network error or timeout, use a fallback mechanism
        if (authError.message === 'network_error' ||
            authError.message?.includes('timeout') ||
            authError.message === '{}' ||
            authError.status === 504) {

          // Store the phone number in localStorage as a fallback
          localStorage.setItem('dev_auth_phone', phoneNumber);

          // Simulate a successful sign-in for development
          toast.showSuccess('DEV MODE: Successfully authenticated with fallback method');
          onClose();

          // Reload the page to simulate a sign-in
...
export default CustomPhoneSignIn;
