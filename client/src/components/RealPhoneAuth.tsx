...
interface RealPhoneAuthProps {
  onClose: () => void;
}

const RealPhoneAuth: React.FC<RealPhoneAuthProps> = ({ onClose }) => {
  ...
  const requestVerificationCode = async (phoneNumber: string) => {
    try {
      console.log(`[Server] Requesting verification code for ${phoneNumber}`);

      // Use our Twilio service to send the verification code
      ...
      // Show the code in a toast message for development
      if (result.code) {
        toast.showInfo(`Your verification code is: ${result.code}`, 15000);
      }
  ...
  const verifyCodeWithServer = async (phoneNumber: string, code: string) => {
    try {
      console.log(`[Server] Verifying code for ${phoneNumber}: ${code}`);

      // Use our Twilio service to verify the code
      const result = twilioService.verifyCode(phoneNumber, code);
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      // Verify code with server
      const result = await verifyCodeWithServer(formattedPhoneNumber, enteredOTP);
