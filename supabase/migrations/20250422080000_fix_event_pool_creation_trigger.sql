-- Fix the event_pool_creation_trigger to ensure it's using platform_fee

-- Drop existing trigger and functions
DROP TRIGGER IF EXISTS event_pool_creation_trigger ON events;
DROP FUNCTION IF EXISTS create_event_pool_trigger() CASCADE;
DROP FUNCTION IF EXISTS initialize_event_pool(uuid) CASCADE;
